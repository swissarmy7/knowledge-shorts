...
const DURATION_TARGET_PROFILE = {
    8: { targetSeconds: 53, minSeconds: 47, maxSeconds: 57 },
    10: { targetSeconds: 51, minSeconds: 46, maxSeconds: 56 },
    12: { targetSeconds: 48, minSeconds: 43, maxSeconds: 54 },
};
    ...
    try {
        const response = await fetchWithAuth(buildAppUrl('api/generate-script'), {
            ...
            body: JSON.stringify({
                ...
                voice_age: selectedVoiceAge === 'young' ? '청년' : (selectedVoiceAge === 'middle' ? '중년' : '아동'),
                voice_tone: selectedVoiceTone === 'bright' ? '밝고 유쾌하게' : (selectedVoiceTone === 'calm' ? '차분하게' : (selectedVoiceTone === 'dark' ? '어둡게' : '슬프게')),
                voice_speed: selectedVoiceSpeed
            }),
        });
...
if (voiceGenderToggle) {
    const opts = voiceGenderToggle.querySelectorAll('.voice-gender-option');
    opts.forEach(opt => {
        opt.onclick = () => {
            opts.forEach(o => o.classList.remove('active'));
            opt.classList.add('active');
            selectedVoiceGender = opt.dataset.gender;
            console.log('Selected Gender:', selectedVoiceGender);
        };
    });
}

// Step 1 Voice Age 토글 바인딩
if (voiceAgeToggle) {
    const opts = voiceAgeToggle.querySelectorAll('.voice-age-option');
    opts.forEach(opt => {
        opt.onclick = () => {
            opts.forEach(o => o.classList.remove('active'));
            opt.classList.add('active');
            selectedVoiceAge = opt.dataset.age;
            console.log('Selected Age:', selectedVoiceAge);
        };
    });
}

// Step 1 Voice Tone 토글 바인딩
if (voiceToneToggle) {
    const opts = voiceToneToggle.querySelectorAll('.voice-tone-option');
    opts.forEach(opt => {
        opt.onclick = () => {
            opts.forEach(o => o.classList.remove('active'));
            opt.classList.add('active');
            selectedVoiceTone = opt.dataset.tone;
            console.log('Selected Tone:', selectedVoiceTone);
        };
    });
}
